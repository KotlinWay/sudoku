...
public class StatsTest {
    ...
    @Test public void победаСчитаетсяИСыграннойИВыигранной() {
        Stats stats = Stats.empty().afterWin(Difficulty.EASY, 300, 0, TODAY).stats;
    ...
    @Test public void доляПобедСчитаетсяОтВсехПартий() {
        Stats stats = Stats.empty().afterLoss()
                .afterWin(Difficulty.EASY, 300, 0, TODAY).stats;
    ...
    @Test public void первоеВремяБезПодсказокСтановитсяЛучшим() {
        Outcome outcome = Stats.empty().afterWin(Difficulty.HARD, 420, 0, TODAY);
    ...
    @Test public void подсказкиЛишаютРекорда() {
        Outcome outcome = Stats.empty().afterWin(Difficulty.HARD, 60, 1, TODAY);
    ...
    @Test public void болееБыстраяПартияОбновляетРекорд() {
        Stats first = Stats.empty().afterWin(Difficulty.HARD, 420, 0, TODAY).stats;

        Outcome second = first.afterWin(Difficulty.HARD, 300, 0, TOMORROW);
    ...
    @Test public void болееМедленнаяПартияРекордНеПортит() {
        Stats first = Stats.empty().afterWin(Difficulty.HARD, 300, 0, TODAY).stats;

        Outcome second = first.afterWin(Difficulty.HARD, 420, 0, TOMORROW);
    ...
    @Test public void рекордыУровнейНеСмешиваются() {
        Stats stats = Stats.empty().afterWin(Difficulty.EASY, 100, 0, TODAY).stats;
    ...
    @Test public void перваяПобедаНачинаетСерию() {
        assertEquals(1, Stats.empty().afterWin(Difficulty.EASY, 60, 0, TODAY).stats.streak);
    }

    @Test public void вчерашняяПобедаПродолжаетСерию() {
        Stats first = Stats.empty().afterWin(Difficulty.EASY, 60, 0, TODAY).stats;

        Stats second = first.afterWin(Difficulty.EASY, 60, 0, TOMORROW).stats;
    ...
    @Test public void втораяПобедаЗаДеньСериюНеУдлиняет() {
        ...
        Stats second = first.afterWin(Difficulty.EASY, 60, 0, TODAY).stats;
    ...
    @Test public void пропущенныйДеньНачинаетСериюЗаново() {
        ...
        Stats second = first.afterWin(Difficulty.EASY, 60, 0, LATER).stats;
    ...
    @Test public void лучшаяСерияПомнитсяПослеОбрыва() {
        Stats stats = Stats.empty()
                .afterWin(Difficulty.EASY, 60, 0, TODAY).stats
                .afterWin(Difficulty.EASY, 60, 0, TOMORROW).stats
                .afterWin(Difficulty.EASY, 60, 0, LATER).stats;
    ...
    @Test public void потраченныеПодсказкиНакапливаются() {
        Stats stats = Stats.empty()
                .afterWin(Difficulty.EASY, 60, 2, TODAY).stats
                .afterWin(Difficulty.EASY, 60, 1, TOMORROW).stats;
